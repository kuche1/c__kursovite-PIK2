...
uint input_positive_uint(char*prompt){
	for(;;){
...
void str__input(char*prompt, char*target, int maxlen){
	
	for(;;){
		
		char*test = input(prompt);
		if(strlen(test) > maxlen){
			printf("Ne moje da e po-dulgo ot %d znaka\n", maxlen);
			continue;
		}
		
		int ind=0;
		for(; ind<strlen(test); ind++){
			target[ind] = test[ind];
		}
		target[ind] = 0;
		break;
...
void str__input_capital_or_num_up_to_20(char*prompt, char*target){
	for(;;){
		//printf(prompt);
		//scanf("%s", target);
		//while(getchar()!='\n');
		str__input(prompt, target, 20);
		
		int failed = 0;
		for(int ind=0; ind<strlen(target); ind++){
			char c = target[ind];
			if( is_char_int(c) || is_char_capital(c) );
			else{
...
void str__input_capital_up_to_50(char*prompt, char*target){
	for(;;){
		//printf(prompt);
		//scanf("%s", target);
		//while(getchar()!='\n');
		str__input(prompt, target, 50);
...
void str__input_EGN(char*prompt, char*target){
	for(;;){
		//char*test = input(prompt);
		//printf(prompt);
		str__input(prompt, target, 10);
